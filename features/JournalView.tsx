...
import { Camera, X, Trash2, MapPin, Save, Edit3 } from 'lucide-react';
import { supabase } from '../supabase';

const JournalView: React.FC<{ tripConfig: any }> = ({ tripConfig }) => {
  const tripId = tripConfig.id;
  const localKey = `jrnl_${tripId}`;
  ...
  const handleSave = async () => {
    ...
    if (supabase) {
      try {
        await supabase.from('journals').upsert(payload);
      } catch (e) {
        console.error(e);
      }
    }
          ...
          <div key={item.id} className="bg-white p-6 rounded-[2.5rem] shadow-soft border-4 border-white animate-in zoom-in-95">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-2"><button onClick={() => { setEditingItem(item); setForm({ content: item.content, location: item.location, imageUrl: item.images[0] }); setShowModal(true); }} className="p-2 text-journey-blue"><Edit3 size={18}/></button><button onClick={() => handleDelete(item.id)} className="p-2 text-journey-red"><Trash2 size={18}/></button></div>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[200] bg-journey-brown/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-t-[4rem] sm:rounded-[3rem] p-10 shadow-2xl space-y-6 max-h-[95vh] overflow-y-auto relative">
            ...
            <button onClick={handleSave} className="w-full bg-journey-red text-white font-black py-6 rounded-[2rem] shadow-lg flex items-center justify-center gap-2 active:scale-95"><Save size={20} /> 發佈回憶</button>
          </div>
        </div>
      )}
    </div>
