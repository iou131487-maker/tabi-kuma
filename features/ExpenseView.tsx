...
const ExpenseView: React.FC<{ tripConfig: any }> = ({ tripConfig }) => {
  ...
  const fetchData = async () => {
    ...
    if (supabase && isSupabaseConfigured) {
      try {
        ...
        if (memData) {
          const finalMem = memData.length > 0 ? memData : [{ name: '我' }];
          setMembers(finalMem);
  ...
  const handleSave = async () => {
    ...
    if (supabase && isSupabaseConfigured) {
      if (editingExpense) supabase.from('expenses').update(payload).eq('id', editingExpense.id).then();
      else supabase.from('expenses').insert([payload]).then();
    }
    closeForm();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    setExpenses(updated);
    localStorage.setItem(`expenses_${tripId}`, JSON.stringify(updated));
    if (supabase && isSupabaseConfigured) supabase.from('expenses').delete().eq('id', id).then();
  };
    ...
    <div className="space-y-6 pb-24">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="space-y-3">
             <div className="flex h-5 w-full bg-white/40 rounded-full overflow-hidden border-2 border-white/20 p-0.5">
                {categorySummary.map(cat => (<div key={cat.name} style={{ width: `${cat.percentage}%` }} className={`${cat.color} h-full transition-all duration-1000`}></div>))}
             </div>
          </div>
        </div>
      </div>
      ...
      <button onClick={() => { closeForm(); setShowForm(true); }} className="w-full bg-white rounded-4xl p-6 shadow-soft flex items-center justify-between active:scale-[0.98] border-4 border-white">
         <div className="flex items-center gap-4">
           <div className="w-14 h-14 rounded-2xl bg-journey-green text-white flex items-center justify-center shadow-soft-sm"><Plus size={28} strokeWidth={3} /></div>
            ...
            <div key={ex.id} className="bg-white rounded-[2.25rem] p-5 flex items-center justify-between shadow-soft border-2 border-journey-sand/5 animate-in slide-in-from-bottom-4 group">
              <div className="flex items-center gap-4 flex-grow overflow-hidden">
                ...
                <div className="overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 mt-0.5"><span className="text-[9px] font-black text-journey-green uppercase">{ex.payer} · {ex.splitCount} 人分擔</span></div>
                </div>
              </div>
              <div className="flex items-center gap-4">
        ...
        <div className="fixed inset-0 z-[100] bg-journey-brown/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-t-[4rem] sm:rounded-[3.5rem] p-10 shadow-2xl space-y-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center">
              <h3 className="text-2xl font-black text-journey-brown uppercase">New Bill</h3>
              <button onClick={closeForm} className="p-3 bg-journey-cream rounded-full text-journey-brown/30"><X size={20} /></button>
            </div>
            <div className="space-y-6">
              <input placeholder="例如：便利商店零食" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-journey-cream p-5 rounded-[2rem] text-sm font-black focus:outline-none" />
              <div className="flex gap-3"><input type="number" placeholder="0" value={amount} onChange={e => setAmount(e.target.value)} className="flex-grow bg-journey-cream p-5 rounded-[2rem] text-2xl font-black focus:outline-none" /><button onClick={() => setCurrency(currency === 'JPY' ? 'HKD' : 'JPY')} className="bg-journey-accent px-8 rounded-[2rem] font-black text-journey-brown shadow-soft-sm">{currency}</button></div>
              <div className="grid grid-cols-2 gap-4">
                 <select value={payer} onChange={e => setPayer(e.target.value)} className="w-full bg-journey-cream p-5 rounded-[1.5rem] text-xs font-black">{members.map(m => <option key={m.id || m.name} value={m.name}>{m.name}</option>)}</select>
                 <input type="number" value={splitCount} onChange={e => setSplitCount(e.target.value)} className="w-full bg-journey-cream p-5 rounded-[1.5rem] text-xs font-black" min="1" />
              </div>
            </div>
            <button onClick={handleSave} className="w-full bg-journey-darkGreen text-white font-black py-6 rounded-[2.5rem] shadow-lg active:scale-95 border-b-4 border-black/10 uppercase tracking-[0.2em] flex items-center justify-center gap-3"><Plus size={20} strokeWidth={3} /> Confirm Bill</button>
          </div>
        </div>
      )}
    </div>
