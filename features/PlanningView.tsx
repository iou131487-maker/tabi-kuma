...
const PlanningView: React.FC<{ tripConfig: any }> = ({ tripConfig }) => {
  ...
  const resetModal = () => {
    setText(''); setPreviewImg(null); setParentId(null); setEditingId(null); setShowAddModal(false);
  };
    ...
    return (
      <div key={item.id} className="space-y-2">
        <div className={`bg-white rounded-[2rem] p-5 flex items-center gap-4 shadow-soft-sm group/item transition-all ${item.completed ? 'opacity-40 grayscale' : 'hover:translate-x-1'}`} style={{ marginLeft: `${depth * 24}px` }}>
           <div onClick={() => toggleStatus(item.id, item.completed)} className="cursor-pointer shrink-0 active:scale-125 transition-transform">
           ...
           <div className="flex-grow flex flex-col gap-1 overflow-hidden">
             <div className="flex items-center gap-2">
               ...
               {item.image_url && <ImageIcon size={14} className="text-journey-blue shrink-0 animate-pulse" />}
             </div>
             
             {item.image_url && (
               <div className="relative group/img cursor-pointer w-fit mt-2" onClick={() => setSelectedImage(item.image_url)}>
                 <div className="w-20 h-20 rounded-2xl overflow-hidden border-4 border-journey-cream shadow-sm active:scale-95 transition-all">
                    <img src={item.image_url} className="w-full h-full object-cover" />
                 </div>
                 <div className="absolute inset-0 bg-black/10 rounded-2xl opacity-0 group-hover/img:opacity-100 flex items-center justify-center transition-opacity">
           ...
           <div className="flex items-center gap-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
             <button onClick={(e) => openEdit(e, item)} className="p-2.5 text-journey-brown/30 hover:text-journey-green hover:bg-journey-green/5 rounded-xl transition-all">
                <Edit2 size={18} />
             </button>
             <button onClick={(e) => handleDelete(e, item.id)} className="p-2.5 text-journey-brown/30 hover:text-journey-red hover:bg-journey-red/5 rounded-xl transition-all">
                <Trash2 size={18} />
             </button>
             <button onClick={(e) => { e.stopPropagation(); setParentId(item.id); setEditingId(null); setShowAddModal(true); }} className="p-2.5 text-journey-green bg-journey-green/10 rounded-xl active:scale-90 transition-all">
                <Plus size={18} strokeWidth={3} />
             </button>
             {hasChildren && (
               <button onClick={(e) => { e.stopPropagation(); toggleExpand(item.id); }} className="p-2.5 rounded-xl bg-journey-cream text-journey-brown/40 ml-1 active:scale-90 transition-all">
    ...
    <div className="space-y-6 pb-28">
      ...
      <div className="space-y-4">
        {loading && items.length === 0 ? (
          ...
          <>
            <div className="space-y-4">
              {items.filter(i => !i.parent_id).length === 0 ? (
                <div className="text-center py-20 bg-white/20 rounded-[3rem] border-4 border-dashed border-journey-sand opacity-40">
                  <p className="font-black text-journey-brown text-sm">還沒有任何計畫...✨<br/>點擊下方按鈕開始吧！</p>
                </div>
              ) : items.filter(i => !i.parent_id).map(item => renderItem(item))}
            </div>
            
            <button 
              onClick={() => { setParentId(null); setEditingId(null); setShowAddModal(true); }} 
              className="w-full py-7 border-4 border-dashed border-journey-sand rounded-[2.5rem] text-journey-brown/30 text-sm font-black flex items-center justify-center gap-4 uppercase tracking-[0.2em] hover:bg-white/40 hover:border-journey-green/40 hover:text-journey-green transition-all active:scale-[0.98] mt-6"
            >
              <Plus size={24} strokeWidth={3} /> 新增主項目
            </button>
          </>
        )}
      </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-[150] bg-journey-brown/95 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in duration-300" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-full max-h-[85vh] animate-in zoom-in-95 duration-500">
            <button className="absolute -top-14 right-0 p-4 bg-white/10 rounded-full text-white hover:bg-white/20 active:scale-90 transition-all">
              <X size={28} />
            </button>
            <img src={selectedImage} className="w-full h-auto max-h-[80vh] object-contain rounded-[3rem] border-8 border-white shadow-2xl" alt="Preview" />
            <p className="text-center text-white/40 text-[10px] font-black uppercase tracking-[0.4em] mt-6">Tap anywhere to close</p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[120] bg-journey-brown/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-t-[4rem] sm:rounded-[3.5rem] p-10 shadow-2xl space-y-8 animate-in slide-in-from-bottom-10 duration-500">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-2xl font-black text-journey-brown uppercase tracking-tighter leading-none">
                  {editingId ? 'Edit Item' : (parentId ? 'New Sub' : 'New Main')}
                </h3>
                <p className="text-[10px] font-bold text-journey-brown/30 uppercase mt-2 tracking-widest">
                  {activeTab === 'todo' ? 'Tasks' : activeTab === 'packing' ? 'Packing' : 'Shopping List'}
                </p>
              </div>
              ...
              <div onClick={() => fileInputRef.current?.click()} className="aspect-video bg-journey-cream rounded-[2.5rem] border-4 border-dashed border-journey-sand flex flex-col items-center justify-center cursor-pointer overflow-hidden relative group transition-all hover:border-journey-green/40">
                 {previewImg ? (
                   ...
                   <div className="text-center group-hover:scale-110 transition-transform">
                      <Camera size={32} className="mx-auto text-journey-sand mb-3" />
                      <p className="text-[10px] font-black opacity-30 uppercase tracking-[0.2em]">Add Reference Photo</p>
                   </div>
                 )}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-journey-brown/30 uppercase tracking-widest ml-4">Content</label>
              <input 
                value={text} 
                onChange={(e) => setText(e.target.value)} 
                placeholder="想要紀錄什麼呢..." 
                className="w-full bg-journey-cream rounded-[2rem] p-6 text-journey-brown font-black focus:outline-none ring-journey-green/30 focus:ring-8 transition-all" 
                autoFocus 
              />
            </div>
            ...
            <button onClick={handleSave} className="w-full bg-journey-darkGreen text-white font-black py-6 rounded-[2.5rem] shadow-lg active:scale-95 border-b-4 border-black/10 transition-all uppercase tracking-[0.2em] flex items-center justify-center gap-3">
              {editingId ? <Send size={20} /> : <Plus size={20} strokeWidth={3} />}
