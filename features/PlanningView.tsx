...
const PlanningView: React.FC<{ tripConfig: any }> = ({ tripConfig }) => {
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      ...
      if (supabase && isSupabaseConfigured) supabase.from('planning_items').update({ text, image_url: previewImg }).eq('id', editingId).then();
    } else {
      const newItem = { id: Date.now().toString(), trip_id: tripId, text, completed: false, type: activeTab, parent_id: parentId, image_url: previewImg, created_at: new Date().toISOString() };
      const updated = [...items, newItem];
      setItems(updated);
      localStorage.setItem(`planning_${tripId}_${activeTab}`, JSON.stringify(updated));
      if (supabase && isSupabaseConfigured) supabase.from('planning_items').insert([newItem]).then();
    }
    resetModal();
  };
  ...
  const toggleStatus = async (id: string, current: boolean) => {
    ...
    setItems(updated);
    localStorage.setItem(`planning_${tripId}_${activeTab}`, JSON.stringify(updated));
    if (supabase && isSupabaseConfigured) supabase.from('planning_items').update({ completed: !current }).eq('id', id).then();
  };
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    ...
    setItems(updated);
    localStorage.setItem(`planning_${tripId}_${activeTab}`, JSON.stringify(updated));
    if (supabase && isSupabaseConfigured) supabase.from('planning_items').delete().eq('id', id).then();
  };
  ...
  const renderItem = (item: any, depth = 0) => {
    ...
    return (
      <div key={item.id} className="space-y-2">
        <div className={`bg-white rounded-[2rem] p-5 flex items-center gap-4 shadow-soft-sm transition-all ${item.completed ? 'opacity-40 grayscale' : ''}`} style={{ marginLeft: `${depth * 24}px` }}>
           ...
           <div className="flex-grow flex flex-col gap-1 overflow-hidden">
             ...
             {item.image_url && <div onClick={() => setSelectedImage(item.image_url)} className="w-20 h-20 rounded-2xl overflow-hidden mt-2 cursor-pointer"><img src={item.image_url} className="w-full h-full object-cover" /></div>}
           </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[120] bg-journey-brown/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-t-[4rem] sm:rounded-[3.5rem] p-10 shadow-2xl space-y-8 animate-in slide-in-from-bottom-10">
            <div className="flex justify-between items-center"><h3 className="text-2xl font-black text-journey-brown">New Item</h3><button onClick={resetModal} className="p-3 bg-journey-cream rounded-full text-journey-brown/30"><X size={20} /></button></div>
            {activeTab === 'shopping' && (<div onClick={() => fileInputRef.current?.click()} className="aspect-video bg-journey-cream rounded-[2.5rem] border-4 border-dashed border-journey-sand flex flex-col items-center justify-center cursor-pointer overflow-hidden">{previewImg ? (<img src={previewImg} className="w-full h-full object-cover" />) : (<Camera size={32} className="text-journey-sand" />)}<input type="file" ref={fileInputRef} className="hidden" onChange={e => { const file = e.target.files?.[0]; if (file) { const r = new FileReader(); r.onloadend = () => setPreviewImg(r.result as string); r.readAsDataURL(file); } }} accept="image/*" /></div>)}
            <input value={text} onChange={(e) => setText(e.target.value)} placeholder="想要紀錄什麼呢..." className="w-full bg-journey-cream rounded-[2rem] p-6 text-journey-brown font-black focus:outline-none" />
